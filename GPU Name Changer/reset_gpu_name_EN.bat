...
cls
echo ================================================
echo          RESET GPU NAME TOOL BY BROWNGRASS
echo ================================================
echo.
echo   %ESC%[93m[1] %ESC%[92mHow to get GPU Hardware ID%ESC%[0m
echo   %ESC%[93m[2] %ESC%[92mReset GPU name to default%ESC%[0m
...
echo   %ESC%[91mIf Method 1 doesn't work, please try Method 2.%ESC%[0m
echo.
...
cls
echo ================================================
echo             RESET GPU NAME TO DEFAULT
echo ================================================
echo.
...
for /f "tokens=*" %%a in ('reg query "HKEY_LOCAL_MACHINE\SYSTEM\CurrentControlSet\Enum" /s /f "%hwid%" ^| findstr "Control"') do (
    reg delete "%%a" /v FriendlyName /f >nul 2>&1
    set found=1
)
...
echo.
echo %ESC%[93mGPU name has been successfully reset to default!%ESC%[0m
